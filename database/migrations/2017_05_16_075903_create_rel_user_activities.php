...
class CreateRelUserActivities extends Migration
{
    ...
    public function up()
    {
         Schema::create('rel_user_activities', function (Blueprint $table) {
            ...
            $table->tinyInteger('status')->default(0)->comment('参加活动，反悔推出活动');
            $table->tinyInteger('is_pay')->default(0)->comment('是否付费');
            $table->integer('price')->default(0)->comment('付费价格/100');
            $table->timestamps();
            $table->softDeletes();
        });
    }
    ...
    {
        Schema::dropIfExists('rel_user_activities');
    }
}
